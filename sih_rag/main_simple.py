...
from crew_pipeline_simple import run_full_pipeline, get_or_create_indexes, reset_memory, pdf_cache
import uvicorn
import os
import tempfile
import asyncio
...
# === FastAPI App ===
app = FastAPI(
    title="Insurance Claim Evaluator - Simplified Version",
    description="Backend for insurance policy evaluation with simplified ML pipeline",
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    The PDF will be indexed for text search.
    """
    try:
        # Validate file type
        ...
        # Warm-up cache (create text index) in background
...
async def query_pdf(
    pdf_id: str = Form(...),
    question: str = Form(...),
    session_id: str = Form("default")  # Session ID parameter
):
    """
...
async def get_sessions():
    ...
    """
    from crew_pipeline_simple import memory_manager
